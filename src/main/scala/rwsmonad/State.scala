...
object State:
  ...
  given [S]: Functor[[A] =>> State[S, A]] with
    extension [A] (fa: State[S, A]) def map[B](f: A => B): State[S, B] =
      s =>
        val (s2, a) = fa(s)
        (s2, f(a))

  given [S]: Applicative[[A] =>> State[S, A]] with
    extension [A, B] (ff: State[S, A => B]) def ap(fa: State[S, A]): State[S, B] =
      s =>
        val (s2, f) = ff(s)
        val (s3, a) = fa(s2)
        (s3, f(a))
  ...
  given [S]: Monad[[A] =>> State[S, A]] with
    extension [A] (fa: State[S, A]) def flatMap[B](f: A => State[S, B]): State[S, B] =
      s =>
        val (s2, a) = fa(s)
        f(a)(s2)
